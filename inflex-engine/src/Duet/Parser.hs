...
parens p = go <?> "parens e.g. (x)"
  where go = do
         _ <- equalToken OpenParen
         e <- p <?> "expression inside parentheses e.g. (example)"
         _ <- equalToken CloseParen<?> "closing parenthesis ‘)’"
         pure e

brackets :: (Location  -> TokenParser a) -> TokenParser a
brackets p = go <?> "brackets e.g. [...]"
  where go = do
         loc <- equalToken OpenBracket <?> "open bracket ‘[’"
