    ...
    go =
      \case
        ...
        UnkindedTypeApp f x -> do
          ...
          case fKind of
            RowKind -> error "TODO: renameField: Kind inference for rows."
                ...
                Nothing ->
                  case find
                         ...
                         (map
                            ...
                            , specialTypesArray (specialsTypes specials)
                            ]) of
                    Just ty -> pure (ty, [])
...
renameType specials tyVars types t = either go pure (isType t)
  where
    go =
      \case
        ...
        UnkindedTypeApp f a -> do
          f' <- go f
          case typeKind f' of
            RowKind -> error "renameType: RowKind"
            FunctionKind argKind _ -> do
              a' <- go a
    ...
    specials' =
      ...
      , setup (specialTypesArray . specialsTypes)
