...
- [ ] Recursive editing - produce an "edit" like a lens modifier?

#+BEGIN_SRC haskell-mode
data Modification
  = OverIndex Int Modification
  | SetCode String
#+END_SRC

The server can then process the modification as either e.g.

- VectorExpression [1,2,3] -- update in place [999,2,3]

- LATER: reverse (VectorExpression [1,2,3]) -- wrapper traversal e.g.
  imap (\i x -> if i == 0 then 999 else x)

For now, the traversal should be disallowed.
