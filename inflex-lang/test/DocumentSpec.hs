...
table_map_defaulting =
  it
    "t = [{x:1,y:2.0}]; k = map(r:r.x*r.y,t)"
    (pendingWith "Seems that the defaulter is inferring the wrong instance (Integer) for the multiply op: InstanceGlobal (IntegerOpInstance MulitplyOp)")
    -- [("t", "[{x:1,y:2.0}]"), ("k", "map(r:r.x*r.y,t)")]
    {-(\[u1, u2] ->
       [ Named
           { uuid = Uuid u1
