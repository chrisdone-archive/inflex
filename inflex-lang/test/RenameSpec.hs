...
spec = do
  it
    ...
    (do shouldBe
          ...
          (Right
             (IsRenamed
                {unresolvedGlobals = mempty,  thing =
                    LiteralExpression
                      (NumberLiteral
                         (Number
        ...
        shouldBe
          ...
          (Right (IsRenamed {unresolvedGlobals = mempty, thing = ApplyExpression (Apply {location = AutogeneratedCursor, function = GlobalExpression (Global {location = AutogeneratedCursor, name = GlobalRef FromDecimalGlobal, scheme = RenamedScheme}), argument = LiteralExpression (NumberLiteral (Number {location = ExpressionCursor, number = DecimalNumber (Decimal {places = 1, integer = 1232}), typ = Just (ConstantType (TypeConstant {location = SignatureCursor TypeCursor, name = IntegerTypeName}))})), typ = Just (ConstantType (TypeConstant {location = SignatureCursor TypeCursor, name = IntegerTypeName}))}), mappings = M.fromList [(ExpressionCursor,SourceLocation {start = SourcePos {line = 1, column = 1, name = ""}, end = SourcePos {line = 1, column = 6, name = ""}}),(SignatureCursor TypeCursor,SourceLocation {start = SourcePos {line = 1, column = 10, name = ""}, end = SourcePos {line = 1, column = 17, name = ""}})]})))
  describe
    "Globals"
    (do it
          "fromInteger"
